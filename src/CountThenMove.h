...
void* ctm_thread(void *args) {
    ...
    for(int i = count_args->start; i < count_args->end; i++) {
        const Tuple *tuple = count_args->input + i;
        int partition = tuple_hash(tuple, count_args->b);

        // write to output buffer and increment offset
        Buffer* buffer = count_args->output_buffers[partition];
        int buffer_offset = offsets[partition];

        buffer_insert(buffer, tuple, buffer_offset);
...
double count_then_move(const Tuple* input, int tuple_size, int thread_count, int hash_bits){
    ...
    Buffer** output_buffers = (Buffer**)malloc(sizeof(Buffer*) * partition_count);
    int buffer_size = ((tuple_size + (partition_count - 1)) / partition_count) * 5;
    for (int i = 0; i < partition_count; i++) {
        output_buffers[i] = create_buffer(buffer_size);
    }
    ...
    for(int i = 0; i < thread_count; i++) {
        ...
        args->input = input; 
        args->output_buffers = output_buffers;
        args->b = hash_bits;
